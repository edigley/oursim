...
import com.edigley.oursim.util.AB;
...
//		String host = "150.165.85.109";
		...
		String workloadPattern = "%s_workload_7_dias_%s_sites_%s_upp_%s.txt";

		String nupp = "10upp";
		
		String resultDir =                  "/local/edigley/mestrado/valor_grades_p2p/outputs/";
		String jobResultDir = resultDir;
		String workloadsDir =               "/local/edigley/mestrado/valor_grades_p2p/workloads/";
		String peerCapacityDescriptionDir = "/local/edigley/mestrado/valor_grades_p2p/grid_capacity/";
		String machinesDescriptionDir = "/local/edigley/mestrado/valor_grades_p2p/machines/";
		String spotInstancesPricesDir =     "/local/edigley/mestrado/valor_grades_p2p/spot_instances_prices/19-01-2014/";
			...
			for (int nSites : nSitesV) {
				for (int upp : nUppV) {
					for (int nRes : nResV) {
						...
						String wFile = String.format(workloadPattern, workloadType, nSites, upp, rodada);
						...
						String oursimTrace = String.format("oursim-trace-%s_%s_machines_7_dias_%s_sites_%s_upp_%s.txt", scheduler, nRes, nSites, upp, rodada);
						oursimTask.outputs.add(oursimTrace);
						if (runOurSim) {
							outputs.add(oursimTrace);
						}
						String uFile = String.format("oursim-trace-utilization-%s_%s_machines_7_dias_%s_sites_%s_upp_%s.txt", scheduler, nRes, nSites, upp, rodada);
						if (utilization) {
							oursimTask.outputs.add(uFile);
							outputs.add(uFile);
						}
						String oursimPattern = java + "oursim.jar -w %s -wt %s -s %s %s -pd %s -nr %s -synthetic_av ourgrid -o %s %s -md %s -prs %s -acs %s -upp %s";// XXX
						// -erw
						// %s";
						...
						oursimTask.cmd = String.format(oursimPattern, inputDir + wFile, workloadType, scheduler, nReplicas, isd, nRes, oursimTrace,
								utilization ? " -u " + uFile : "", md, peerRankingSeeds[rodada], avCharacterizationSeeds[rodada], upp);
						...
						for (String sptFilePath : spts) {
							for (int spotLimit : spotLimits) {
								...
								String spotsimTrace = String.format("spot-trace-%s_%s_machines_7_dias_%s_sites_%s_upp_%s_spotLimit_groupedbypeer_%s_av_%s_%s.txt",
										scheduler, nRes, nSites, upp, spotLimit, groupedbypeer, sptFilePath, rodada);
								spotsimTask.outputs.add(spotsimTrace);
								outputs.add(spotsimTrace);
								String uSpotFile = String.format(
										"spot-trace-utilization-%s_%s_machines_7_dias_%s_sites_%s_upp_%s_spotLimit_groupedbypeer_%s_av_%s_%s.txt", scheduler, nRes,
										nSites, upp, spotLimit, groupedbypeer, sptFilePath, rodada);
								if (utilization) {
									spotsimTask.outputs.add(uSpotFile);
									outputs.add(uSpotFile);
								}
								String spotsimPattern = java + "spotsim.jar -spot %s -l %s -bid max -w %s -av %s -o %s %s -pd %s -md %s -ait %s -upp %s";
...
//		FileUtils.copyFileToDirectory(new File("/home/edigley/local/resources_BKP/exemplo-de-execucao.txt"), new File(inputDir));
		...
		for (String inputFile : inputs) {
			...
			} else if (inputFile.startsWith("machines_speeds_")) {
				sourceDir = machinesDescriptionDir;
			} else if (inputFile.startsWith("iosup_site_description_")) {
		...
		if (runOurSim) {
			...
			cmdXargsSB.append("cat args.txt | xargs -n27 -t -P4 java -Xms500M -Xmx4000M -XX:-UseGCOverheadLimit -jar oursim.jar");
		}else{
			//cmdXargsSB.append("cat args.txt | xargs -n17 -t -P15 java -Xms250M -Xmx1000M -XX:-UseGCOverheadLimit -jar spotsim.jar");
			cmdXargsSB.append("cat args.txt | xargs -n19 -t -P4 java -Xms500M -Xmx4000M -XX:-UseGCOverheadLimit -jar spotsim.jar");
		}
