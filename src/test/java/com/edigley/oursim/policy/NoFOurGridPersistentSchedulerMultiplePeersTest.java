...
public class NoFOurGridPersistentSchedulerMultiplePeersTest extends AbstractOurSimAPITest {
...
//		peers.add(new Peer("peer_" + 1, 1, Processor.EC2_COMPUTE_UNIT.getSpeed(), NoFSharingPolicy.getInstance()));
//		peers.add(new Peer("peer_" + 2, 1, Processor.EC2_COMPUTE_UNIT.getSpeed(), NoFSharingPolicy.getInstance()));

		for (int i = 0; i < 50; i++) {
			peers.add(new Peer("peer_" + i, 50, Processor.EC2_COMPUTE_UNIT.getSpeed(), NoFSharingPolicy.getInstance()));
		}
